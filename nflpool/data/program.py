...
def division_standings():
    ...
    response = requests.get(
        (url), auth=HTTPBasicAuth(secret.msf_username, secret.msf_pw)
    )
    ...
    for division in data["divisionteamstandings"]["division"]:
        for team in division["teamentry"]:
            ...
            cur.execute(
                """INSERT INTO division_standings(team_id, rank)
                VALUES(?,?)""",
                (team_id, rank),
            )
...
def playoff_standings():
    ...
    for conference in data["playoffteamstandings"]["conference"]:
        for team in conference["teamentry"]:
            ...
            cur.execute(
                """INSERT INTO playoff_rankings(team_id, rank)
                VALUES(?,?)""",
                (team_id, rank),
            )
...
def player_stats():
    ...
    for data in stats:
        ...
        cur.execute(
            """INSERT INTO player_stats(player_id, passyds, rushyds, recyds, sacks, interceptions, season)
            VALUES(?,?,?,?, ?, ?, "2016")""",
            (player_id, passyds, rushyds, recyds, sacks, interceptions),
        )
...
def conference_stats():
    ...
    for conference in data["conferenceteamstandings"]["conference"]:
        for team in conference["teamentry"]:
            ...
            cur.execute(
                """INSERT INTO conference_standings(team_id, rank, points_for, pr_td, kr_td)
                VALUES(?,?,?,?,?)""",
                (team_id, rank, points_for, pr_td, kr_td),
            )
